...
def debug_current_geometry():
    ...
    # Get raw IFC spaces directly for geometry extraction
    raw_ifc_spaces = ifc_file.by_type('IfcSpace')
    print(f"[OK] Found {len(raw_ifc_spaces)} raw IFC spaces")
    
    # Extract geometry
    geom_extractor = GeometryExtractor()
    
    # Process first 10 spaces for detailed analysis
    sample_spaces = raw_ifc_spaces[:10]
